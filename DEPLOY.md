...
A página sobe em um container só: o **Caddy**, que serve os arquivos e emite o
certificado HTTPS sozinho (Let's Encrypt). Não há build, não há Dockerfile —
a pasta `site/` é montada direto no container.
...
```bash
cp .env.example .env
nano .env      # preencha DOMINIO e EMAIL_TLS, salve com Ctrl+O e saia com Ctrl+X
docker compose up -d
```
...
**Cuidado ao limpar o Docker**
O volume `caddy_data` guarda os certificados. `docker compose down -v` apaga
esse volume e força a reemissão de tudo. Para parar o site sem perder nada,
use `docker compose down` sem o `-v`.
